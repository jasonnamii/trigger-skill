...
## 2026-04-28 · v1.0.1 PATCH (제출청소 BAN_WORDS 박제)
- **변경:** `references/protocol-cleanup.md` 축3 내부언어 강화
- **추가 박제 8종:** 분석라벨 축계열(N축·축별·다축·수렴축) / 그룹카운터(N대 동인·요인·원칙) / 항목카운터(N건) / 동학계열(인물·조직·시장 동학) / 엘베피치·엘리베이터 피치 / 확신도 / 모드라벨(LIGHT·DEEP·TURBO) / §+영문코드(§AI·§A-1·§H)
- **추가 후보 5종:** DNA·경력 DNA / Phase N·P1·P2 / MUST·SHOULD·MAY / SCOPE_OUT / 자체검사·자체점검
- **판정 룰 강화:** 숫자+한글자 카운터 정규식 `[0-9]+(축|건|대|종|개|차원|영역|항목)` hit = 자동 평문 풀어쓰기 강제
- **경계 명시:** DSL 본문 사용 OK, 산출물·대화 출력 금지
- **출처:** UP v110 §[NO_WORK_LABEL] BAN_WORDS 확장 요청 (형 픽)
